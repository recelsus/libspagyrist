...
- 選択処理: built-in selector、`fzf` selector、number selector、fallback selector
...
通常の出力先はstdoutです。editor出力を明示した場合、libspagyristは以下の順に
エディタを探します。
...
`VISUAL` または `EDITOR` に `nvim -R` のようなエディタ固有オプションを設定できます。
ただし、`--output "nvim -R"` のように出力先へエディタコマンドを直接指定することは
対象外です。利用側CLIでは `--output editor` のように出力先だけを指定します。

## Help Text

クライアントアプリケーションは、libspagyristの共通help断片を自分のhelpに合成できます。
選択肢は一覧APIとしても取得できるため、selector、format、outputの追加時は
libspagyrist側の一覧を更新するだけでクライアント側へ反映しやすくなります。
...
`runtime_info_text()` は selector と editor の状態を表示します。selectorでは
内部実装の `builtin` / `number` と、外部コマンド `fzf` の検出状態を返します。
editorでは `VISUAL`、`EDITOR`、`nvim`、`vim`、`vi`、`nano` の順で状態を返します。
より細かく組み立てたい場合は、以下を利用できます。
...
標準的なクライアントでは built-in selector を既定にし、非TTY環境などで利用できない
場合に number selector へfallbackする構成を推奨します。`fzf` などは明示的に選択する
外部selectorとして利用できます。
...
Unicode は UTF-8 文字列を破壊しない byte-based 処理です。built-in selectorの
文字入力は構造的に完結したUTF-8バイト列を1文字としてqueryへ追加し、不完全な
UTF-8入力は文字入力として扱いません。書記素単位のカーソル移動、全角幅計算、
Unicode 正規化は初期実装では対象外です。
...
built-in selectorの強調表示は、matcherのバイト位置を表示文字列上のUTF-8コードポイント
単位へ変換して行います。同一コードポイント内に複数の一致バイトがあっても1回だけ
強調します。不正なUTF-8候補文字列はクラッシュさせず、構造的に読めないバイト列を
1バイト単位として扱います。表示幅はバイト数ベースの上限であり、全角文字のセル幅には
完全対応していません。
...
`match_result::positions` は候補文字列上のバイトオフセットです。
`ranked_candidate::search_positions` は `candidate_text::search` 上のバイトオフセット、
`ranked_candidate::display_positions` は `candidate_text::display` 上のバイトオフセットです。
